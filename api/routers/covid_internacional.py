...
def get_country_coord(country):
    results = list(db[f"Covid_Cases_Checked"].find({"Country/Region": country},{"Lat":1,"Long":1,"_id":0}))
    return loads(json_util.dumps(results))

@router.get("/covid_internacional/data/{country}/{data}")
def get_country_data(country, data):
    results = list(db[f"Covid_{data.title()}_Checked"].find({"Country/Region": country},{"_id":0}))
    return loads(json_util.dumps(results))
